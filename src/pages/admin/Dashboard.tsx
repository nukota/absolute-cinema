...
const Dashboard = () => {
  ...
  const { data: dashboardData, isLoading: loading } =
    useDashboardStats(selectedMonth);
  ...
  const lineData = useMemo(() => {
    if (!dashboardData) return [];
    return dashboardData.daily_data.map((data) => ({
  ...
  const genrePieData = useMemo(() => {
    if (!dashboardData) return [];
    return dashboardData.genre_distribution.map((item) => ({
      name: item.genre,
  ...
  const topMoviesData = useMemo(() => {
    if (!dashboardData) return [];
    return dashboardData.top_movies.map((movie) => ({
                ...
                >
                  {genrePieData.map((_, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        ["#667eea", "#00d4ff", "#f093fb", "#4facfe", "#fa709a"][
                          index % 5
                        ]
                      }
                    />
