...
const CreateRoomDialog: React.FC<CreateRoomDialogProps> = ({
    ...
    for (let gridRow = 1; gridRow <= 13; gridRow++) {
      for (let gridCol = -6; gridCol <= 6; gridCol++) {
        ...
        const isInCenterArea =
          gridRow >= 2 && gridRow <= 12 && gridCol >= -4 && gridCol <= 4;
  ...
  const getSeatsWithLabels = () => {
    ...
    const activeRows = [...new Set(activeSeats.map((s) => s.gridRow))].sort(
      (a, b) => a - b
    );
    const activeColumns = [
      ...new Set(activeSeats.map((s) => s.gridColumn)),
    ].sort((a, b) => a - b);

    // Create mapping from grid position to display position
    const rowMap = new Map(
      activeRows.map((gridRow, index) => [gridRow, index])
    );
    const colMap = new Map(
      activeColumns.map((gridCol, index) => [gridCol, index])
    );
