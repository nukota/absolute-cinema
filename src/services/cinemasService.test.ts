...
import { cinemasApi } from "./cinemasService";
...
describe("cinemas API", () => {
  ...
  it("gets all cinemas and one cinema by ID", async () => {
    server.use(
      http.get("http://localhost:8000/cinemas", () => HttpResponse.json([cinema])),
      http.get("http://localhost:8000/cinemas/cinema-1", () => HttpResponse.json(cinema)),
    );

    await expect(cinemasApi.getAllCinemas()).resolves.toEqual([cinema]);
    await expect(cinemasApi.getCinemaById(cinema.cinema_id)).resolves.toEqual(cinema);
  });

  it("updates a cinema and deletes it", async () => {
    server.use(
      http.patch("http://localhost:8000/cinemas/cinema-1", async ({ request }) => {
        expect(await request.json()).toEqual({ name: "Renamed Cinema" });
        return HttpResponse.json({ ...cinema, name: "Renamed Cinema" });
      }),
      http.delete("http://localhost:8000/cinemas/cinema-1", () =>
    ...
    await expect(cinemasApi.deleteCinema(cinema.cinema_id)).resolves.toBeUndefined();
  });
});
